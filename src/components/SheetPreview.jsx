...
const SheetPreview = ({ doc, activeSheet, onSheetChange }) => {
  ...
  const sheetNames = sheetData?.sheet_names || (doc?.sheet_names) || [];

  // Load sheet data whenever doc or activeSheet changes
  useEffect(() => {
    ...
    fetchSheetData({ docId: doc.doc_id, sheetName: activeSheet, offset: 0, limit: PAGE_SIZE })
      .then((data) => {
        setSheetData(data);
        // Propagate the full sheet_names list back if caller doesn't have it yet
        if (data.sheet_names?.length && !doc.sheet_names) {
          // nothing to do — caller manages sheet state
        }
        // Auto-default to first sheet if backend returned different sheet_names
        if (data.sheet_names?.length && !data.sheet_names.includes(activeSheet)) {
          onSheetChange(data.sheet_names[0]);
        }
      })
      .catch((err) => setError(err.message))
      .finally(() => setLoading(false));
  }, [doc?.doc_id, doc?.status, activeSheet]);

  // When doc changes and we don't have an activeSheet yet, pick the first sheet
  useEffect(() => {
    if (!doc?.doc_id || !doc.sheet_names?.length) return;
    if (!activeSheet || !doc.sheet_names.includes(activeSheet)) {
      onSheetChange(doc.sheet_names[0]);
    }
  }, [doc?.doc_id, doc?.sheet_names]);
  ...
  const displaySheetNames = sheetData?.sheet_names || doc?.sheet_names || [];

  // No doc or not yet ready
  if (!doc) {
    return (
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      {/* Sheet tab bar */}
      <div className="border-b border-slate-200 px-3 py-2 overflow-x-auto shrink-0">
        {displaySheetNames.length <= 1 ? (
          <span className="inline-block px-3 py-1 text-xs font-medium rounded-md bg-blue-100 text-blue-800 border border-blue-200 whitespace-nowrap">
            {displaySheetNames[0] || activeSheet || doc.name}
          </span>
        ) : (
      ...
      <div className="flex-1 overflow-auto relative" ref={tableRef}>
        ...
        {error && (
          <div className="p-4 text-xs text-red-600 bg-red-50 border-b border-red-200">{error}</div>
        )}
      ...
      {sheetData && sheetData.total_rows > PAGE_SIZE && (
        <div className="border-t border-slate-200 px-4 py-2 flex items-center justify-between text-xs text-slate-500 shrink-0 bg-white">
          <span>
            {offset + 1}–{Math.min(offset + PAGE_SIZE, sheetData.total_rows)} of{' '}
